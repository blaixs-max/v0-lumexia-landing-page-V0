              ...
              {phases.map((phase, index) => (
                <div key={phase.id} className="flex items-center flex-1 last:flex-none">
                  <button onClick={() => setActivePhase(phase.id)} className="flex flex-col items-center gap-2 group">
                    ...
                    <div
                      className={`relative w-14 h-14 sm:w-16 sm:h-16 rounded-full flex items-center justify-center text-lg sm:text-2xl font-bold transition-all duration-500 glass ${
                  ...
                  {index < phases.length - 1 && (
                    <div
                      className="flex-1 h-1 mt-6 min-w-4 rounded-full"
                      style={{
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {phases
              ...
              ?.steps.map((step, index) => {
                const phaseColor = phases[activePhase - 1].color
                return (
                  <div
                    key={index}
                    ...
                    style={{
                      borderColor: step.hasFire ? phaseColor : undefined,
                      boxShadow: step.hasFire 
                        ? `0 0 30px ${phaseColor}30, inset 0 0 20px ${phaseColor}10` 
                        : undefined,
                    }}
                  >
                    ...
                    <div className="relative z-10 flex flex-col h-full">
                      <div className="flex items-center justify-between mb-5">
                        ...
                        >
                          <step.icon className="w-7 h-7" style={{ color: phaseColor }} />
                        </div>
                        <div
